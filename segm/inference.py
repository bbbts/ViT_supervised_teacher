...
# ---------------------------------------------------------
# Convert segmentation map to RGB using dataset category colors
# ---------------------------------------------------------
...
def main(model_path, input_dir, output_dir, dataset, gpu, gt_dir):
    ...
    # -------------------------------------------------
    # Automatically infer architecture from checkpoint
    # -------------------------------------------------
    embed_dim = state_dict["encoder.pos_embed"].shape[-1]
    ...
    if embed_dim == 192:
        ...
        raise ValueError(f"Unknown backbone embedding size: {embed_dim}")
    
    n_cls = state_dict["decoder.cls_emb"].shape[1] 
    print("Detected classes:", n_cls)
    ...
    model_cfg = cfg["model"][backbone].copy()
    
    model_cfg["backbone"] = backbone
    
    decoder_cfg = cfg["decoder"][decoder].copy()
    decoder_cfg["name"] = decoder
    
    decoder_cfg["n_cls"] = state_dict["decoder.cls_emb"].shape[1]
    ...
    model_cfg["image_size"] = (grid * patch_size, grid * patch_size)
    
    # ----------------------------
    # CREATE MODEL
    # ----------------------------
    print("MODEL CFG n_cls:", model_cfg["n_cls"])
    print("DECODER CFG n_cls:", model_cfg["decoder"].get("n_cls", None))
    ...
    gt_freq = gt_count / (total_pixels + 1e-6)
    fw_iou = np.sum(gt_freq * per_class_iou)
